...
if(isset($_POST['usapp'])){
  if(isset($_POST['status'])){
    ...
    $us= $_POST['usapp'];
$sql = "update `tbl_student` set active=".$ustatus." where  username='".$us."' ";
$con->query($sql);
}
...
<!--  page banner -->
  ...
  <div class="col-sm-2" style="float:left;"> <a href="approvalreq.php"><button type="submit" class="btn btn-default"><span class="glyphicon glyphicon-bell"></span> &nbsp;Approval Requests (<?php
   $sql = "select * from tbl_student where approve=0";
...
echo $k;
 ...
 <div class="row">
   ...
   <h2 class="page-banner-heading  text-center">Admin Panel:  </h2><p> All Students:</p>
   </div>
   <div class="col-sm-2"></div>
</div>
  </section> 
...
<div class="col-sm-10">
  <?php
  $sql ="select * from tbl_student where approve=1 order by active desc";
 $result = $con->query($sql);
...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
        ...
        echo '<td>'.'<form action="viewuser.php" method="post"> <input type="hidden" name="userid" value="'.$row['id'].'" /><input type="hidden" name="fromprj" value="'.$row['id'].'" /><input  type="submit" value="View" class="btn-default" /></form>'.'</td>';
        echo '<td>'.'<form action="edituser.php" method="post"> <input type="hidden" name="userid" value="'.$row['id'].'" /><input type="hidden" name="fromprj" value="'.$row['id'].'" /><input  type="submit" value="Edit" class="btn-default" /></form>'.'</td>';
